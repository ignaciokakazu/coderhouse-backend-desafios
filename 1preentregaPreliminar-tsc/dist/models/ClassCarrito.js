...
class ClassCarrito {
  async getCarritoById(id) {
    try {
      this.lista = await _db.db.read("carrito");
      const prod = this.lista.filter(product => product.id == id);

      if (!prod.length) {
        throw new Error('No existe el producto por ID');
      }
  ...
  async getCarritoAll() {
    ...
    //si existe el id
    try {
      this.carrito = await _db.db.read("carrito");
      return this.carrito;
    } catch (error) {
      return {
        error: error.message
  ...
  async deleteCarritoById(id) {
    try {
      ...
      if (!this.carrito.length) {
        throw new Error('Carrito vacío. No se puede eliminar');
      }

      const filtrada = this.carrito.filter(prod => prod.id != id);

      if (filtrada.length == this.carrito.length) {
        throw new Error('No se encuentra el ID');
      }

      this.carrito = filtrada;
      await _db.db.write("carrito", this.carrito);
      return {
        msg: "Eliminado"
      };
    } catch (error) {
  ...
  async deleteCarritoAll() {
    try {
      ...
      _db.db.write("carrito", this.carrito);
  ...
  async addCarritoById(id) {
    try {
      //Productos
      this.listaProductos = await _db.db.read("productos");
      const prod = this.listaProductos.filter(product => product.id == id);

      if (!prod.length) {
        throw new Error('No hay productos disponibles. Comuniquese con el administrador');
...
const Carrito = new ClassCarrito();
